...
namespace DOL.Tests.Integration.Database
{
	...
	public class CustomParamsTest
	{
		...
		public void TableParamSaveLoadTest()
		{
			...
			ClassicAssert.IsFalse(TestData.IsPersisted, "Newly Created Data Object should not be persisted...");
			ClassicAssert.IsFalse(TestData.CustomParams.First().IsPersisted, "Newly Created Param Object should not be persisted...");
			
			// Insert Object
			var paramsInserted = TestData.CustomParams.Select(o => Database.AddObject(o)).ToArray();
			var inserted = Database.AddObject(TestData);
			
			ClassicAssert.IsTrue(inserted, "Test Object not inserted properly in Database !");
			ClassicAssert.IsTrue(paramsInserted.All(result => result), "Params Objects not inserted properly in Database !");
			
			// Check Saved Object is Persisted
			ClassicAssert.IsTrue(TestData.IsPersisted, "Newly Created Data Object should be persisted...");
			ClassicAssert.IsTrue(TestData.CustomParams.First().IsPersisted, "Newly Created Param Object should be persisted...");
			...
			ClassicAssert.IsTrue(RetrieveData.IsPersisted, "Retrieved Data Object should be persisted...");
			ClassicAssert.IsTrue(RetrieveData.CustomParams.First().IsPersisted, "Retrieved Param Object should be persisted...");
			
			// Compare both Objects
			ClassicAssert.AreEqual(TestData.ObjectId, RetrieveData.ObjectId, "Newly Created and Inserted Data Object should have the same ID than Retrieved Object.");
			
			ClassicAssert.AreEqual(TestData.CustomParams.Length,
			                RetrieveData.CustomParams.Length,
			                "Saved Object and Retrieved Object doesn't have the same amount of Custom Params");
			
			ClassicAssert.AreEqual(TestData.CustomParams.First(param => param.KeyName == "TestParam").Value,
